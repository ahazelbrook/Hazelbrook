...
That asymmetry is the whole problem, and it exists because the base case has no owner. The option has a sponsor, the cost estimate has a quantity surveyor, the demand model has a modeller with a name on the report. The counterfactual is written last, by whoever is assembling the chapter, and it is the only input where being wrong in the direction of the project does not look like advocacy. It looks like housekeeping.

There are two directions of error. The first, and by far the more common, is a base case that is too thin. Australian guidance is consistent that the counterfactual is a *do-minimum*, not a do-nothing: continued routine and periodic maintenance, asset renewal and like-for-like replacement of life-ending components, and projects already committed and funded. Strip those out and the case starts manufacturing benefits. Leave a committed project out and its benefits are counted twice — once in its own business case, and once in yours.
...
**What a reviewer looks for.** The switching value: not the BCR at 7%, but the rate at which it reaches 1.0, and how far that sits from the central rate. Then the same question of demand and of capital cost. A case that states its own switching values is nearly always the stronger case for having done so.
